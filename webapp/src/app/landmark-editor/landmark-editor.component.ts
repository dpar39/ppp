...
export class LandmarkEditorComponent implements OnInit {
  ...
  private _inputPhoto: string = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAUAAAAFCAYAAACNbyblAAAAHElEQVQI12P4//8/w38GIAXDIBKE0DHxgljNBAAO9TXL0Y4OHwAAAABJRU5ErkJggg==";
  ...
  @ViewChild('photo')
  m_imgElmt: ElementRef;

  @ViewChild('container')
  m_containerElmt: ElementRef;

  @ViewChild('crownMark')
  m_crownMarkElmt: ElementRef;

  @ViewChild('chinMark')
  m_chinMarkElmt: ElementRef;
  ...
  calculateViewPort(): void {
      this.m_viewPortWidth = this.m_containerElmt.nativeElement.width;
      this.m_viewPortHeight = this.m_containerElmt.nativeElement.height;
  };

  renderImage(): void {
        let xw = this.m_imageWidth * this.m_ratio;
        let yh = this.m_imageHeight * this.m_ratio;
        this.m_imgElmt.nativeElement.style.width.px = xw;
        this.m_imgElmt.nativeElement.style.height.px =  yh;
